...
namespace App\Classes\hybridauth\src;

use App\Classes\hybridauth\src\Exception\InvalidArgumentException;
use App\Classes\hybridauth\src\Exception\UnexpectedValueException;
use App\Classes\hybridauth\src\Storage\StorageInterface;
use App\Classes\hybridauth\src\Logger\LoggerInterface;
use App\Classes\hybridauth\src\Logger\Logger;
use App\Classes\hybridauth\src\HttpClient\HttpClientInterface;
...
class Hybridauth
{
    ...
    public function getAdapter($name)
    {
        ...
        $adapter = isset($config['adapter']) ? $config['adapter'] : sprintf('App\\Classes\\hybridauth\\src\\Provider\\%s', $name);
            ...
            foreach ($fs as $file) {
                if (!$file->isDir()) {
                    ...
                    if ($name === mb_strtolower($provider)) {
                        $adapter = sprintf('App\\Classes\\hybridauth\\src\\Provider\\%s', $provider);
                        break;
                    }
                }
            }
            if ($adapter === null) {
                throw new InvalidArgumentException('Unknown1 Provider.');
            }
        }
